...
// NOTE: Using traits to differentiate between thread-safe and non-thread-safe
// resulted in a significant tradeooff. DOnt know why
...
template <int N = 624> class MersenneTwisterRNGThreadSafe {
private:
  ...
  double MT_genrand() {
    ...
    double result =
        ((double)y * 2.3283064365386963e-10); /* reals: [0,1)-interval */
    mtx.unlock();
    return result;
  }

public:
  MersenneTwisterRNGThreadSafe(int seed) : initial(true), mti(0) {
...
template <int N> class MersenneTwisterRNG {
private:
  unsigned int i_seed[N + 1];
  int mti;
  bool initial;
...
void gen_rand(double *data, size_t n, Random::MersenneTwisterRNG<624> &mt) {
  for (size_t i = 0; i < n; i++) {
    data[i] = mt.runif();
  }
}
...
Rcpp::NumericVector MT_CPP(unsigned int seed, unsigned int n,
  ...
  if (n_cores == 1) {
    ...
    Random::MersenneTwisterRNG<624> mt(seed);
    ...
    std::vector<Rcpp::NumericVector> results(n_cores);
    std::vector<Random::MersenneTwisterRNG<624>>
        rngs; // NOTE: way more efficient than using only one instance of
              // MersenneTwisterRNGThreadSafe

    // Initialize RNGs for each thread
    for (unsigned int i = 0; i < n_cores; ++i) {
      rngs.emplace_back(seed +
                        i); // NOTE: Use of different seeds for each thread
    }

    const int n_sizes = std::floor(n / n_cores);
    int remainder = n % n_cores;

    for (size_t i = 0; i < n_cores; i++) {
      int size = n_sizes + (i < remainder ? 1 : 0);
      results[i] = Rcpp::NumericVector(size);
    }

    for (size_t i = 0; i < n_cores; i++) {
      threads[i] = std::thread(gen_rand, results[i].begin(), results[i].size(),
                               std::ref(rngs[i]));
    }
    ...
    for (const auto &result : results) {
      std::copy(result.begin(), result.end(), v.begin() + index);
      index += result.size();
    }
